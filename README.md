# 📌 Automated Customer Returns Portal

## 🚀 Project Impact
- Reduced return processing time from 3–5 days to 4 hours (proposed)
- Designed automated end-to-end returns workflow
- Created BRD, BPMN, and User Stories for implementation

## 📖 Project Overview
This project demonstrates Business Analyst skills by designing an automated returns portal for an e-commerce company to replace a manual process.


### 👤 Author
Laxmi Rushaali Kuravi  
March 2026  

---

## 📖 Project Overview
This project demonstrates Business Analyst skills by designing an automated customer returns portal for an e-commerce company. The goal is to replace a manual 3–5 day process with a 4-hour automated workflow.

---

## 🎯 Key Skills Demonstrated
- BRD Writing  
- Process Mapping (BPMN)  
- Requirements Analysis  
- Stakeholder Identification  

---

## 🧠 Day 1 — BA Role Understanding (Summary)
Analyzed Business Analyst job descriptions and identified key skills like requirements gathering, process mapping, and stakeholder communication. Identified gaps in BRD writing, Agile, and tools like JIRA.

---

## 🧠 Day 2 — BRD Learning (Summary)
Learned the structure of a Business Requirements Document including scope, stakeholders, functional requirements, non-functional requirements, constraints, assumptions, and success metrics.

---

## 📄 Day 3 — Business Requirements Document

### 🎯 Scenario
An e-commerce company wants to automate its manual returns process.

### Executive Summary
The current return process takes 3–5 days and causes delays and errors. The proposed solution is an automated returns portal to reduce processing time to 4 hours and improve customer satisfaction.

### Project Scope
**IN Scope:**
- Online return requests  
- Refund processing  
- Email notifications  

**OUT of Scope:**
- In-store returns  
- International orders  

### Functional Requirements
- The system shall allow customers to submit returns online  
- The system shall send confirmation emails  
- The system shall allow tracking of return status  

### Non-Functional Requirements
- System shall load within 2 seconds  
- Availability of 99.9%  
- Secure data handling  

---

## 🔄 Day 4 — Process Mapping with BPMN

### What is BPMN?
BPMN stands for Business Process Model and Notation. It is a standard way of visualizing business processes using symbols like events, tasks, and gateways.

### My Process Flow Diagram
<img width="952" height="228" alt="image" src="https://github.com/user-attachments/assets/473417cb-830c-411b-8b7e-5f2619055841" />


### Reflection
Created a BPMN diagram using Lucidchart AI with Customer, System, and Warehouse swimlanes. Learned how gateways represent decision points such as payment success or failure.

---

## 🚀 Tools Used
- Lucidchart  
- GitHub  
